...
def finishline(rect, movement, tiles):
    # hit_list = collision_test(rect, movement, tiles)
    ...
    for row in game_map:
        ...
        for tile in row:
            if tile == '1':
                ...
                turret_number += 1
                snowball_rect_list.append(x * tile_size[0] + 27)
...
# player movement & collisions
    ...
    for x in range(0, len(snowball_rects)):
        ...
        hit_list = collision_test(snowball_rects[x], tile_rects)
    ...
    for event in pygame.event.get():
     ...
     #   if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
            # if we click the left mouse button, the first thing we want to do is blit a snowball (with original position
            # should be the same as the players current position) -> PLAYER_rect x and y
            # end position is where the mouse is
         #   end_position = pygame.mouse.get_pos()

            # make a snowball rectangle, and blit it on the window
            # drawing the snowball at the initial position
         #   screen.blit(snowball, (PLAYER_rect.x, PLAYER_rect.y))

            # move the snowball along the line from our start position to the end position
            # end_position -> (x,y) end_position[0], end_position[1]

            # want snowball to shoot from player position to mouse position
           # pygame.Rect.move(end_position[0] - PLAYER_rect.x, end_position[1] - PLAYER_rect.y)
